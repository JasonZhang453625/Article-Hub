...
class LocaleStrings {
  ...
  static const _en = LocaleStrings(
    ...
    apiConfigDesc:
        'Enter your OpenAI-compatible API credentials. Provider keys never enter account sync. Full JSON backups include them, so store exported files securely.',
    baseUrl: 'Base URL',
    apiKey: 'API Key',
    model: 'Model',
    ...
    usingBuiltIn:
        'Vector search uses your own OpenAI-compatible embedding endpoint and key.',
    ...
    backupDesc:
        'Export all articles, filters and settings to JSON, including provider keys. Store the file securely. Importing merges into current data.',
    export: 'Export',
    import: 'Import',
  ...
  static const _zh = LocaleStrings(
    ...
    apiConfig: 'API 配置',
    apiConfigDesc: '输入你的 OpenAI 兼容 API 配置。密钥不参与账号同步；完整 JSON 备份会包含密钥，请安全保管导出文件。',
    baseUrl: '基础 URL',
    apiKey: 'API 密钥',
    model: '模型',
    ...
    usingBuiltIn: '向量检索需要配置你自己的 OpenAI 兼容 Embedding 地址与密钥。',
    ...
    data: '数据',
    backupRestore: '备份与恢复',
    backupDesc: '导出所有记忆、筛选和设置到 JSON 文件（包含 provider 密钥），请安全保管。导入会合并到当前数据。',
    export: '导出',
    import: '导入',
    importBackup: '导入备份',
